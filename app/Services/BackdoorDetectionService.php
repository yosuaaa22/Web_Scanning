...
private function analyzeSuspiciousFunctions($html)
{
    $suspiciousFunctions = [
        ...
        'information_disclosure' => [
            'phpinfo', 'posix_getpwuid', 'posix_getgrgid', 'get_current_user',
            'getcwd', 'getmyuid', 'getmygid'
        ],
        'network_functions' => [
...
private function isLikelySensitiveData($value)
{
    // Expanded exclusions
    $exclusions = [
        'example', 'test', 'sample', 'demo', 'dummy',
        'username', 'password', 'user@example.com',
        '123-45-6789', '123.456.7890'
    ];
...
private function enhancedFormAnalysis(Crawler $crawler)
{
    ...
    $crawler->filter('form')->each(function ($node) use (&$formRisks) {
        ...
        $node->filter('input')->each(function ($input) use (&$formRisks) {
            ...
            $name = $input->attr('name');
            if (in_array($type, ['password', 'hidden']) ||
...
private function analyzeEventHandlers(Crawler $node, &$scriptRisks)
{
    $suspiciousHandlers = [
        ...
        'onkeypress', 'onkeydown', 'onkeyup', 'onsubmit', 'onbeforeunload'
    ];
